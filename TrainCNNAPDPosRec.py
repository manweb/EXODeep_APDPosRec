...
def deepcnn(x, trainEnergy = False):
	...
	HPool2 = max_pool(HConv2, [2,3])

	# Flatten structure
	HPool2_flat = tf.reshape(HPool2, [-1, 8320])

	# First hidden layer
	W0 = weight_variable("W0", [8320,1000])
	b0 = bias_variable([1000])
	H0 = tf.nn.relu(tf.matmul(HPool2_flat,W0)+b0)
	...
	# Second hidden layer
	W1 = weight_variable("W1", [1000,500])
	b1 = bias_variable([500])
	H1 = tf.nn.relu(tf.matmul(H0_drop,W1)+b1)

	# Third hidden layer
	W2 = weight_variable("W2", [500,200])
	...
	if trainEnergy:
		nOutput = 1
	W4 = weight_variable("W4", [100,nOutput])
	...
	reg_term = regularization*(tf.nn.l2_loss(WConv0)+tf.nn.l2_loss(bConv0)+tf.nn.l2_loss(WConv1)+tf.nn.l2_loss(bConv1)+tf.nn.l2_loss(WConv2)+tf.nn.l2_loss(bConv2)+tf.nn.l2_loss(W0)+tf.nn.l2_loss(b0)+tf.nn.l2_loss(W1)+tf.nn.l2_loss(b1)+tf.nn.l2_loss(W2)+tf.nn.l2_loss(b2)+tf.nn.l2_loss(W3)+tf.nn.l2_loss(b3)+tf.nn.l2_loss(W4)+tf.nn.l2_loss(b4))
