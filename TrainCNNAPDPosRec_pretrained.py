...
from tf_model_medium import tf_model_medium
...
def deepcnn(x, regularization, trainEnergy = False):
	...
	HPool3 = max_pool(HConv3, [3,3])

	# Flatten structure
	HPool3_flat = tf.reshape(HPool3, [-1, 2048])

	# First hidden layer
	W0 = weight_variable("W0", [2048,1024])
	...
	# Second hidden layer
	W1 = weight_variable("W1", [1024,256])
	b1 = bias_variable([256])
	H1 = tf.nn.relu(tf.matmul(H0_drop,W1)+b1)

	# Third hidden layer
	#W2 = weight_variable("W2", [512,128])
	...
	if trainEnergy:
		nOutput = 1
	W3 = weight_variable("W3", [256,nOutput])
...
def main(optimizer = 'Adam', numEpochs = None, batchSize = 500, maxTrainSteps = 10000, learningRate = 0.001, regTerm = 1e-5, dropout = 1.0, outDir = './output/', trainingSet='', testSet='', model='', trainEnergy = False, prefix='', savePredPlots = False, scaleLabels = False, pretrained = False):
	print("#################################################")
	...
	with graph.as_default():
		...
		if pretrained or not model:
			if pretrained:
				if not model:
					print('Please specify a model')
					return

				net = tf_model_medium(model, x_image)

				# Using pretrained model. Select any layer of the network and add
				# additional layers to it
				# ***************************************************************

				# Get the first fully connected layer and add two more layers
				# This represents the original network but the last fully connected
				# layers are re-trained
				fc5 = net['fc4']

				W0 = weight_variable("W0", [1024,256])
        			b0 = bias_variable([256])
        			H0 = tf.nn.relu(tf.matmul(fc5,W0)+b0)

				nOutput = 3
        			if trainEnergy:
                			nOutput = 1
        			W1 = weight_variable("W1", [256,nOutput])
        			b1 = bias_variable([nOutput])
        			y = tf.matmul(H0,W1)+b1

				reg_term = regularization*(tf.nn.l2_loss(W0) + tf.nn.l2_loss(b0) + tf.nn.l2_loss(W1) + tf.nn.l2_loss(b1))

				# ***************************************************************

			if not model:
				y, keep_prob, reg_term = deepcnn(x_image, regularization, trainEnergy)
	...
	with tf.Session(graph=graph) as sess:
		...
		try:
			...
			for i in range(maxTrainSteps):
				...
				feed_dict = {x: batch_x, y_: batch_y, regularization: regTerm}
				if not pretrained:
					feed_dict[keep_prob] = dropout

				sess.run(train_step, feed_dict=feed_dict)

				feed_dict[regularization] = 0
				if not pretrained:
					feed_dict[keep_prob] = 1.0
				current_loss = sess.run(loss, feed_dict=feed_dict)
				if i%10==0:
					feed_dict[x] = cv_set_x
					feed_dict[y_] = cv_set_y
					cv_loss, predicted = sess.run([loss, y], feed_dict=feed_dict)
...
if __name__ == '__main__':
	...
	if not args.eval:
		main(args.optimizer, args.numEpochs, args.batchSize, args.maxTrainSteps, args.learningRate, args.regTerm, args.dropout, args.outDir, args.trainingSet, args.testSet, args.model, args.trainEnergy, args.prefix, args.savePredPlots, args.scaleLabels, args.pretrained)
